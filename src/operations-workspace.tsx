...
export function AttendanceWorkspace({ notify, workspace, userId, previewMode, role }: { notify: (message: string) => void; workspace: WorkspaceData; userId?: string; previewMode: boolean; role: Role }) {
  const monitoringOnly = role === "Admin Daerah" || role === "Admin Desa";
  const [tab, setTab] = useState<"fill"|"recap">(monitoringOnly ? "recap" : "fill");
  const [groupId, setGroupId] = useState(workspace.groups[0]?.id ?? "");
  const [classId, setClassId] = useState(workspace.classes.find(item=>item.is_active&&(!monitoringOnly||item.group_id===groupId))?.id ?? "");
  ...
  useEffect(()=>{if(monitoringOnly&&tab!=="recap")setTab("recap");},[monitoringOnly,tab]);
  useEffect(()=>{if(monitoringOnly&&(!groupId||!workspace.groups.some(item=>item.id===groupId)))setGroupId(workspace.groups[0]?.id??"");},[monitoringOnly,groupId,workspace.groups]);
  const visibleClasses=workspace.classes.filter(item=>item.is_active&&(!monitoringOnly||item.group_id===groupId)&&(role!=="Pengajar"||previewMode||allowedClassIds.includes(item.id)));
  ...
  return <>
    <div className="attendance-page-head"><div><span>{monitoringOnly?"MONITORING PRESENSI":"PRESENSI KELAS"}</span><h1>{monitoringOnly?"Rekap Presensi Wilayah":"Presensi & Rekap"}</h1><p>{monitoringOnly?"Pantau presensi kelas dalam wilayah naungan. Pengisian tetap dilakukan PJ Kelompok/Pengajar.":"Isi presensi harian dan pantau kelengkapan satu bulan."}</p></div></div>
    {!monitoringOnly?<div className="ops-tabs"><button className={cx(tab==="fill"&&"active")} onClick={()=>setTab("fill")}>Isi Presensi</button><button className={cx(tab==="recap"&&"active")} onClick={()=>setTab("recap")}>Rekap Bulanan</button></div>:null}
    {!monitoringOnly&&tab==="fill"?<section className="attendance-fill-card"><div className="attendance-controls-pro"><label><span>Kelas</span><select value={classId} onChange={event=>setClassId(event.target.value)}>{visibleClasses.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span>Tanggal</span><KeyboardInput type="date" value={date} max={localToday()} onChange={event=>setDate(event.target.value)}/></label></div><div className="attendance-student-list">{students.map(student=><article key={student.id}><span className="roster-avatar">{initials(student.full_name)}</span><span><strong>{student.full_name}</strong><small>{date}</small></span><div className="attendance-choice">{(["H","I","A"] as Attendance[]).map(value=><button key={value} className={cx(records[student.id]===value&&"active",value==="A"&&"danger")} onClick={()=>setRecords(current=>({...current,[student.id]:value}))}>{value}</button>)}</div></article>)}{!students.length?<div className="class-empty">Belum ada siswa pada kelas ini.</div>:null}</div><button className="save-class-roster" disabled={saving||!students.length} onClick={()=>void submit()}><Check size={18}/>{saving?"Menyimpan…":"Simpan presensi"}</button></section>:<section className="attendance-recap-workspace"><div className="attendance-recap-filters">{monitoringOnly?<label><span>Kelompok</span><select value={groupId} onChange={event=>{setGroupId(event.target.value);setClassId("");}}>{workspace.groups.map(group=>{const village=workspace.villages.find(item=>item.id===group.village_id);return <option key={group.id} value={group.id}>{role==="Admin Daerah"&&village?`${village.name} · ${group.name}`:group.name}</option>})}</select></label>:null}<label><span>Kelas</span><select value={classId} onChange={event=>setClassId(event.target.value)} disabled={!visibleClasses.length}>{visibleClasses.length?visibleClasses.map(item=><option key={item.id} value={item.id}>{item.name}</option>):<option value="">Belum ada kelas</option>}</select></label><label><span>Bulan</span><KeyboardInput type="month" value={month} onChange={event=>setMonth(event.target.value)}/></label></div><div className="recap-metrics"><article><strong>{expectedDates.length}</strong><span>Jadwal pengajian</span></article><article><strong>{submittedDates.size}</strong><span>Presensi terisi</span></article><article><strong>{journalDates.size}</strong><span>Jurnal terisi</span></article><article className={missingDates.length?"warning":"success"}><strong>{missingDates.length}</strong><span>Belum diisi</span></article><article><strong>{avg}%</strong><span>Rata-rata hadir</span></article></div><div className="individual-recap-table"><div className="individual-recap-head"><strong>Rekap per individu</strong><small>Hadir, izin, alpha, dan persentase bulan terpilih.</small></div>{perStudent.map(item=><article key={item.student.id}><span className="roster-avatar">{initials(item.student.full_name)}</span><span className="person-name"><strong>{item.student.full_name}</strong><small>{item.total} catatan presensi</small></span><span><b>{item.hadir}</b><small>Hadir</small></span><span><b>{item.izin}</b><small>Izin</small></span><span><b>{item.alpha}</b><small>Alpha</small></span><span className="presence-rate"><b>{item.percent}%</b><small>Rata-rata</small></span></article>)}</div></section>}
  </>;
}

export function PptTemplateManager({ notify, workspace, classId, previewMode }: { notify: (message: string) => void; workspace: WorkspaceData; classId: string; previewMode: boolean }) {
